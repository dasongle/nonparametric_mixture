...
\[
\biggl\|
T - \sum_{j=1}^k \widehat\lambda_j \widehat{\phi}_j^{\otimes 3}
\biggr\| \leq 55\eps_T,
...
Given the observations $\mathcal{D}_{X_1X_2X_3}=\{(x_1^i, x_2^i, x_3^i)\}_{i\in[m]}$ drawn \emph{i.i.d.} from a multi-view latent variable model $\mathbb{P}(X_1, X_2, X_3)$, let the kernel matrix associated with $X_1$, $X_2$ and $X_3$ be $K$, $L$ and $G$ respectively and the corresponding feature map be $\phi$, $\psi$ and $\upsilon$ respectively.

we have the empirical estimation of the second/third-order embedding as
%
\begin{eqnarray*}
\widehat\Ccal_{X_1 X_2} = \frac{1}{m} \sum_{i=1}^m \phi(x_1^i)\otimes \psi(x_2^i) = \frac{1}{m}\Phi \Psi^\top\\
\widehat\Ccal_{X_3 X_1} = \frac{1}{m} \sum_{i=1}^m \upsilon(x_1^i)\otimes \phi(x_2^i) = \frac{1}{m}\Upsilon \Phi^\top\\
\widehat\Ccal_{X_2 X_3} = \frac{1}{m} \sum_{i=1}^m \psi(x_1^i)\otimes \upsilon(x_2^i) = \frac{1}{m}\Psi \Upsilon^\top\\
\widehat\Ccal_{X_1 X_2 X_3}:= [\bm{I}_n; \Phi, \Psi, \Upsilon] =
