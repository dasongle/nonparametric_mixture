...
\end{align}
Once we have the conditional embedding $\mu_{X|h}$, we can compute the conditional expectation of a function $f\in \Fcal$ as $\int_{\Xcal} f(x)\,dx = \inner{f}{\mu_{X|h}}_{\Fcal}$. For data from $\RR^d$, if certain kernels are chosen, such as the normalized Gaussian RBF kernel $k(x,x') = \exp(-\|x-x'\|^2/(2 s^2)) / \sqrt{(2\pi s)^d}$, the conditional density $p(x|h)$ can be approximated by the embedding, $\widetilde{p}(x|h) = \inner{\phi(x)}{\mu_{X|h}}_{\Fcal}=\EE_{X|h}[k(x,X)]$. Essentially, this is the convolution of the conditional density with the kernel function. If the kernel bandwidth $s \rightarrow 0$, one can show that the approximation error (bias) $|\widetilde{p}(x|h)-p(x|h)|$ goes to 0~\cite{Wasserman06}.
...
\paragraph{Remark 2:} \Le{for density estimation; bias-variance tradeoff:mention}
