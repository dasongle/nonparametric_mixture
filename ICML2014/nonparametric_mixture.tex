...
\vskip -0.05in
\icmlauthor{Bo Dai, Bo Xie}{bodai,bxie33@gatech.edu}
...
%
\begin{proof}
Sketch: Our proof is different from those in~\citet{AnandkumarEtal:tensor12} which only analyze the perturbation of the tensor decomposition. Our proof further takes into account the error introduced by the approximate whitening step, and its effects to the tensor decomposition.
\vspace{-4mm}
\end{proof}
...
{\bf Remark 2:} Theorem~\ref{thm:samplebound} also gives us an error bound for estimating the integral of a function $f\in \Fcal$ with respect to a mixture component in unsupervised fashion. Under the conditions specified in the theorem, we have 
\begin{align*}
  &\abr{\int_{\Xcal} f(x)\, d\PP(x|h) - \inner{f}{\widehat \mu_{X|h}}_{\Fcal}} \\
  \leq& \nbr{f}_{\Fcal} \nbr{\mu_{X|h} - \widehat \mu_{X|h}}_{\Fcal}
  = O\rbr{\smallfrac{1}{\sqrt{m}}}
\end{align*}
assuming $\nbr{f}_{\Fcal}$ is bounded and $\rho/\sigma_k=O(1)$. We are not aware of any other result of the similar type in this unsupervised setting. 
...
$
assuming $\rho/\sigma_k=O(1)$ and using $\rho = O(s^{-d})$.
Under the conditions specified in Theorem~\ref{thm:samplebound}, we combine the analysis for the two sources of errors, and obtain the bound
\begin{align*}
  |p(x|h) - \widehat p(x|h)|
  = O(s^2 + s^{-d/2} m^{-1/2})
\end{align*}
...
\vspace{-3mm}
{\small
L.S. is supported in part by NSF IIS1116886, NSF/NIH BIGDATA 1R01GM108341, NSF CAREER IIS1350983 and Raytheon Faculty Fellowship.
A.A. is supported in part by  Microsoft Faculty Fellowship, NSF CAREER CCF1254106, NSF CCF1219234, NSF BIGDATA IIS1251267 and ARO YIP Award W911NF-13-1-0084.
}
